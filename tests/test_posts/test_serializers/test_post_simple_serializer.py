...
class PostSimpleSerializerTest(TestCase):
    ...
    def test_with_thumbnail(self, mock_create_download_presigned_url) -> None:
        """썸네일 key가 있으면 post_id/title/thumbnail(URL)로 직렬화"""
        mock_create_download_presigned_url.return_value = (
    ...
    @patch("apps.core.storage.s3.services.s3_svc.create_download_presigned_url")
    def test_without_thumbnail(self, mock_create_download_presigned_url) -> None:
    ...
    @patch("apps.core.storage.s3.services.s3_svc.create_download_presigned_url")
    def test_many(self, mock_create_download_presigned_url) -> None:
