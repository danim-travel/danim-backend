...
google_login_schema = extend_schema(
    tags=["social-login"],
    summary="구글 로그인 시작",
    description="구글 OAuth 인가 페이지로 리다이렉트합니다. (브라우저에서 직접 접근)",
    responses={
        302: OpenApiResponse(description="구글 인가 페이지로 리다이렉트"),
    },
)
...
google_callback_schema = extend_schema(
    ...
    responses={
        ...
        400: OpenApiResponse(
            description="유효하지 않은 요청(state 불일치) 또는 구글 인증 실패"
        ),
    },
)
